...
import { isContentPin, scopeModeOf } from "@/api/kalaidoscope/context-items";
...
/**
 * Fold a tagged/picked item into a context selection — the one pin path,
 * shared by the composer's @-mentions and the bar's pin picker, so tagging
 * something makes it appear in the bar. Rules:
 *
 * - kind+id duplicates and the markers are ignored;
 * - a `Type` is focus only (no longer a pin kind) — ignored;
 * - a fragment or colour onto a Full selection is ignored: it is already in
 *   the context in full, and Full + content pins is not a selectable state;
 * - everything else is appended. Onto `[]` (Off) the pin is the whole context.
 */
export function withContextItem(
  ...
  if (isContentPin(item) && scopeModeOf(items) === "full") return items;
  return [...items, item];
}
