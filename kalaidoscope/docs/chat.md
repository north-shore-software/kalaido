...
`PrepareLLMPrompt`: `HydrateDeltaHistory` walks every stored + new message in order — system messages become `WindowNotice` (if they carry a bounded `window`) plus the hydrated **delta** between the previous `pinned_ids` and this one (added rendered in the conversation's mode, removed named); other messages are `Flatten`ed (`context.md` § 7). The mode is `ConversationSummaries(allMsgs)` = the `summaries` flag of the **latest** `context_spec` on the transcript, applied to every delta in the history — a conversation that switches modes re-renders its whole context in the new mode on the next turn. An empty hydrated transcript → 400 `messages required`.
...
Model = `ResolveRoleFor(RoleChat, conversation.model)`; none → 500. `CheckPromptFits` refuses with **422** and the guard's message; in full mode the message is suffixed with a hint to turn on summaries.
...
Selected per § 3. The model sees rows instead of bodies and is given two tools built on the discover reader (`discover.md` § 3.3) with a **12 fragment reads per turn** budget and chat wording: `read_thing {ids}` and `read_fragment {ids}`.
...
3. Dispatch each call through the reader; an unknown tool yields `no tool named …`. Each result is sent as `tool-output-available` and persisted as a `tool-<name>` part `{toolCallId, toolName, input, output, state: "output-available"}`. **The assistant row is written after every round**, so an interrupted turn keeps its reads.
...
- No server-side tools in full mode; no memory beyond the persisted transcript; no summarisation of long histories — the size guard refuses instead.
