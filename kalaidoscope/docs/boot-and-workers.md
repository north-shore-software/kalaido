...
All workers share one shape: a package-level `chan struct{}` of capacity 1 as the wake signal (a `Signal()` that finds the buffer full is a no-op, so bursts coalesce into one drain), a loop goroutine started in `Register`, and a drain that re-derives its worklist from the database each pass. None holds an in-memory queue of work items. The mapping and discover workers additionally expose in-memory in-flight flags (`Annotating`, `Consolidating`, `LastDrainError`, `Running`, `Pending`) that `GET /api/organize` reads (`organize.md` § 2); these describe the running process only.
...
| **Reconcile wave worker** | `reconcile.Register` (`go workerLoop()`); also sets `engine.RequestWave` | `EnqueueWave()` — `POST /api/reconcile`, and a refinement commit of a chain-marked candidate | a fresh staleness evaluation (`rotation.md`) | **Disabled** by the compile-time constant `waveEnabled = false` (exposed as `WaveEnabled()`, surfaced by `organize.md` § 6): `EnqueueWave` logs and returns without signalling. When enabled: a signal arriving mid-wave coalesces into one follow-up wave; the first entity error ends the wave; `ErrPreempted` retries; `ErrLensNotReady`/`ErrGenerationInFlight` skip the entity | `rotation.md` § 3 |
| **Mapping annotate worker** | `mapping.Register` (`go loop()`) | two signals into one channel: `mapping.Signal()` (a **full** drain: annotate, then one map cycle) from `POST /api/map` and the post-import chain; `mapping.SignalAnnotate()` (annotate only, no cycle) from every non-import fragment create, every ingest completion without `organize_after`, and once at boot (`OnServe`, after `Next`) when any live fragment lacks an annotation. A pending full request survives coalescing: the flag is consumed at the start of the next drain | every live fragment without a `fragment_annotation` row | up to 100 fragments annotated concurrently; a fragment that fails is skipped for the rest of the drain; `ErrExhausted` stops the drain; each call retries `ErrPreempted` indefinitely and quota/transient provider errors up to 6 times; the drain's first error is kept as `LastDrainError` (cleared by a clean drain) and passed to follow-ups | `map.md` § 2–3 |
...
| **Discover worker** | `discover.Register` (`go loop()`) | `discover.Signal(kind)` — `POST /api/discover`, and the post-import chain; unknown kinds are ignored | the pending-kind set, drained in the fixed order colours → projections → reflections | one run per kind; a run's error is logged and the next kind still runs; a run refuses before writing a row when the map has no things, or (projections, reflections) when no colour exists; each model call retries `ErrPreempted` indefinitely and quota/transient errors up to 6 times; run outcome is recorded on the `discover_run` row | `discover.md` § 2 |
...
**Follow-up queue.** `mapping` and `discover` each own a `followup.Queue`: a mutex-guarded slice of `func(error)` callbacks. `AfterDrain(fn)` appends; the loop `Take`s the whole slice at the start of a drain and runs every callback with the drain's error once it ends. A callback registered *during* a drain therefore runs after the *next* one. The only registrant is the post-import chain (`organize.md` § 7), which signals the three discover kinds after a clean mapping drain; the discover queue has no registrant.
...
|---|---|---|---|
| **Ingest batch processing** | `OnRecordCreate("ingest")` hook, after the record is saved | parses every uploaded file, writes fragments, then updates the `ingest` row's `status`/`ingested`/`error` and either signals an annotate-only drain or starts the post-import chain (`ingestion.md` § 3) | the row stays `pending`; nothing resumes it |
...
| **Colour preview fan-out** | `POST /api/colours/preview` | one goroutine per fragment (20), joined inside the request; tied to the request context, so a disconnect cancels them | — |
| **Queue-status writer** | `registerQueueStatus` (`OnServe`): resets the `llm_queue_status` singleton to empty at boot, then subscribes to scheduler changes | each change is debounced 300 ms and written to the row (`state`, `running`, `waiting`, `held`); out-of-order deliveries are dropped by version | the row describes the process, not the workspace; reset at next boot |
...
- Staleness (`rotation.md`) and the organise status (`organize.md`) are computed on demand per request; there is no periodic evaluator.
...
- There is no cron: `time.Tick` in the aggregate loop is the only timer-driven work; the scheduler's wake-up timer and the queue-status debounce are one-shot `AfterFunc`s serving those two mechanisms.
