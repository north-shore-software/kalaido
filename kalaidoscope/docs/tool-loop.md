...
- `FormatToolResults(results)` returns a user message whose content is the outputs joined by a blank line (`"\n\n"`). Results are not labelled with the call they answer; the model relies on order and on each tool's own output format (for the map reader, each card and block names its id, § 7). When every output is empty (a nil `Dispatch`, § 2.4) the message content is only the separators.
...
| `IDsArg(call)` | `ids` (array of string), else `id` | `ids` as given (elements not trimmed; `ids` wins when both keys are present); when `ids` is empty and `id` is non-blank, a one-element slice holding the trimmed `id` | `nil` / empty slice (unmarshal errors ignored) |
...
`New(app, budget)` calls `sourcedata.LoadMapIndex(app)` (a load error is returned and no reader is built) and copies its four parts onto the exported fields:

- `Doc` — the `kalaidoscope_map` row's `body` (the first row by filter `1=1`, limit 1) parsed by `mapdoc.Parse` into a `mapdoc.Document` (`things`, `relationships`, `narrative`). When no map row exists, or the body is blank, `{}`, `null`, not a JSON object, lacks a `things` key or does not unmarshal, the document is empty; the parse's failure flag is discarded by the loader, and `things`/`relationships` are never nil after a successful parse.
- `Version` — the row's `version`, or `0` when there is no row. The reader stores it and never reads it.
- `Rows` — every `fragment_annotation` row ordered by `created` (no liveness filter on the annotation side) as a `sourcedata.Row` (`= prompts.AnnotationRow`: `FragmentID`, `Date`, `Title`, `Summary`, `Things`; a `things` field that fails to unmarshal becomes nil), with `Date` taken from `sourcedata.FragmentDates` — the `occurred_at` of live (`deleted_at = ''`) fragments formatted `YYYY-MM-DD`; a row whose fragment is deleted or has no `occurred_at` gets `""`. Rows are then sorted by `Date` ascending (stable, so `""` dates sort first, then by `created` load order).
- `ByThing` — `sourcedata.IndexRows`: for each row and each of its `things` citations, the citation's `ref` (falling back to `name`) is resolved through `Document.Resolve` (exact `id`, then normalised name or alias); each resolved thing id maps to the ascending list of row indexes citing it, at most one entry per row even when a row cites the same thing twice. Citations that resolve to nothing are dropped.
...
`ReadThings(refs)`: when more than `prompts.DiscoverReadThingLimit` (`10`) refs are given, the list is truncated to the first ten and `prompts.DiscoverTooManyThings(10)` is emitted as the first line. Each ref is trimmed and answered by `ReadThing`; blank refs are not skipped and answer with the no-match line. The answers are joined by single newlines. An empty `refs` produces an empty string.

`ReadThing(ref)` resolves `ref` through `Doc.Resolve` (exact id via `Find`, then normalised name or alias via `FindByName`; normalisation lowercases, collapses whitespace and strips trailing punctuation). No match returns `prompts.DiscoverNoThing(ref)`. Otherwise it returns `prompts.DiscoverThingCard` built from:

- the thing's `id`, `name`, `kind`, `aliases`, `blurb`, and `first_seen`/`last_seen` when `first_seen` is set (the document's own `fragments` count and `exemplar_ids` are not used);
...
- a month timeline: each citing row's `Date` truncated to `YYYY-MM`, counted; a date shorter than seven characters (including `""`) is counted under `prompts.DiscoverUndated` (`undated`); the card lists the keys in lexical order, so `undated` follows every month;
...
### 8.1 Discover (`internal/discover/loop.go`, `context.go`, `reads.go`)

The run context embeds a `*mapreader.Reader` built by `New(app, maxFragmentReads)` with `maxFragmentReads = 12` — the same number as chat's, but with the "per run" exhaustion wording. Discover does not call `Reader.Registry()` or `ChatReadTools()`; it declares its own tools with the discover descriptions: `read_thing` via `IDsTool`, `read_fragment` via `IDTool` with the single parameter `id` (bound to `Reader.ReadFragment(ctx, IDArg(call))`), `list_existing` and `coverage` via `EmptyTool`, `finish` via `IDTool` with parameter `summary`, and `read_colour` via `IDsTool` (handled for every flow but advertised only by flows that name it in `Flow.Tools`; its handler samples member rows with `mapreader.ThingRowSample`); one flow also composes a schema with `StringArraySchema`. The advertised list is the five shared tools in that order followed by the flow's own. The `Runner` runs with `MaxRounds = maxRounds` (`30`), no `StopBeforeLastDispatch`, `Generate` calling `usage.GenerateWithToolCalls` under `usage.RetryThrottled` (`llm-queue-quota.md` § 4) with the `map` role, counting the round and remembering the reply text, `Dispatch = reg.Dispatcher(flow.Dispatch)` so a flow's proposing tools are the fallback (a flow handler's text is returned with `done=false`; its optional output record is collected on the context; its error ends the loop), `OnRoundEnd` saving run progress, and no `PromptGuard` or `OnToolDispatched`. `finish` is the one handler returning `done=true`; it stores the run summary from the last reply text, falling back to `StrArg(call, "summary")` when that text is blank. `Run`'s error is returned to the run's caller. What each flow lists, covers, proposes and creates is in `discover.md` § 3 to § 6.

### 8.2 Explore summaries turn (`internal/explore/summaries.go`)

The turn builds `NewChatReader(app)` (a load error is returned before any stream is opened), takes `reg = reader.Registry()` and advertises `reg.Tools()` — exactly the two tools of § 7.2. The `Runner` runs with `MaxRounds = maxExploreToolRounds` (`4`), `StopBeforeLastDispatch = true`, `Dispatch = reg.Dispatcher(...)` whose fallback answers an unknown tool name with `prompts.DiscoverUnknownTool(name)` (never an error), `OnToolDispatched` sending each output to the client as a `tool-output-available` event and collecting it as a persisted `tool-<name>` part via `chat.ToolResultPart` (a part whose JSON cannot be marshalled is dropped without notice), `PromptGuard = llm.CheckPromptFits` over `llm.MessagesChars` of the updated transcript (`context.md` § 6; a failure is logged and sent as an `error` event), and `OnRoundEnd` persisting the collected parts (only when a conversation record was given and there are parts). `Generate` streams the model with the `chat` role: round 0 drains a completion started before the runner; later rounds start a new stream, with the tool list withheld (`nil`) on the last round so that round can only produce text, and a stream-start failure is logged, sent as an `error` event and returned to the loop. Each round's non-empty reply text is collected as a `text` part. `Run`'s return value is discarded (`_ = runner.Run(...)`); the turn then persists once more, sends `finish` and returns nil regardless of how the loop ended. Seeding, the stream shape and how persisted reads are replayed on later turns are in `explore.md` § 5 and § 6.
...
- The reader does not filter `Rows` or fragment reads by liveness, does not use `Version`, does not use the `ctx` it is handed, and never writes.
