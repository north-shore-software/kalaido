...
3. Building blocks first. A question that was researched and then decided is a projection on its own; so is one project, one argument, one decision with its rationale. Create such a block directly when it is already clear from the map, or recurse when it deserves its own framed exploration — a recurse brief names what to explore, not just node names. contextNodes are the nodes it runs through: grounding and logging, not a boundary on what the fork may read.
4. When recurse returns, compose. Anything broader that would include a block — a strategy that rests on several decisions, an overview of a project made of sub-efforts — should take those blocks as sourceProjections rather than re-reading their fragments, adding nodes only for ground the blocks don't cover. The root exploration finishes last and may compose across every branch.
