...
3. Building blocks first. A question that was researched and then decided is a projection on its own; so is one project, one argument, one decision with its rationale. A block is something the user would open and read on its own — one fragment can legitimately feed several of them when it covers several things. When two candidates overlap and one would contain the other, make the narrower one first and build the broader one on it: a broader entity references and builds on its sources, it does not restate them. Create a block directly when it is already clear from the map, or recurse when it deserves its own framed exploration — a recurse brief names what to explore, not just node names. contextNodes are the nodes it runs through: grounding and logging, not a boundary on what the fork may read.
4. When recurse returns, compose. Anything broader that would include a block — a strategy that rests on several decisions, an overview of a project made of sub-efforts — takes those blocks as sourceProjections. Give every entity the minimum context its brief needs: if the brief needs only a source projection's content, take the projection and not the colours it was built from; if it also needs those colours because they carry material the projection didn't use, take both. The root exploration finishes last and may compose across every branch.
...
Writing a brief: it becomes the entity's lens verbatim. Write it as the instruction that produces the content — "Summarise…", "Lay out…", "Compare…", "State the decision and its rationale…" — addressed to whoever will generate the view from the sources, not as a description of the view. Name the thing the entity is about; give dates only where they matter. When the entity has source projections, name them in the brief by their exact names ("drawing on the projection 'Pricing model' for the decision itself…"): the generator sees each source projection as a block headed by its name, so naming it is how you say what to build on and what carries the weight, versus raw fragments that are there for detail.
...
{"name": "Commercial strategy", "brief": "Lay out the overall commercial strategy, building on the projection 'Pricing model' for the pricing decision and the projection 'Acquisition hooks' for the lead-magnet analysis — reference them rather than restating them — and add the distribution material from the sources not covered by either.", "sourceProjections": ["<id of Pricing model>", "<id of Acquisition hooks>"], "nodes": [{"dimension": "business", "name": "distribution"}]}
