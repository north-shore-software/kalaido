...
- Reflections need a temporal shape to be worth creating: use the journal's own period structure as the evidence — a story that recurs entry after entry with a steady rhythm supports a recurring window; a story confined to one dense stretch supports a single bounded window; only give a reflection a windowSpec where the journal clearly shows that shape. Not every reflection needs a window, but consider it — don't default every entity to a projection.
...
const (
	...
	OrganizeCreateReflectionToolDescription = "Create a reflection (a time-windowed view) now, scoped to one or more map nodes, or the whole workspace if you are the root exploration. Only give it a windowSpec where the journal's time structure clearly supports one. Content generation starts immediately, in the background."
