...
func MapMarkupPrompt(mapBody, fragmentBlock string) string {
	return `You are annotating one fragment from a user's personal workspace so it can be folded into the workspace map: a living index of the workspace's themes, entities, and story.
...
{"summary": string, "themes": [string], "entities": [string]}
...
- "themes": the themes this fragment touches. Prefer the exact "name" of an existing map theme whenever one fits; invent a new short theme name only when nothing in the map covers it.
...
- Only add and refine. Never drop a theme or entity that earlier material still supports; fold spelling and naming variants into "aliases" instead of creating near-duplicate themes.
- Keep themes at a useful grain: broad enough that each covers a meaningful share of the workspace, specific enough to mean something.
