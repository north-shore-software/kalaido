...
build_schema_db() {
  rm -rf "$SCHEMA_ROOT"
  mkdir -p "$SCHEMA_DATA"
  (cd "$KS" && go run ./cmd/sidecar schema bootstrap --dir "$SCHEMA_DATA" >/dev/null)
}

# Freeze the launch schema: copy schema/canonical.go into schema/baseline/v1.go
# so the parity test can build "a database created by version 1" forever after.
# Only meaningful while Version is 1 — after launch the baseline is history and
# every schema change is Canonical + a delta.
cmd_schema_freeze() {
  local version src dst
  version="$(awk '/^const Version = / { print $4; exit }' "$KS/schema/version.go")"
  [[ "$version" == "1" ]] || die "schema:freeze is for the launch baseline only (Version is $version, not 1)"
  src="$KS/schema/canonical.go"
  dst="$KS/schema/baseline/v1.go"
  say "freeze schema v1"
  {
    cat <<'EOF'
// Code generated by ./kalaido.sh schema:freeze from schema/canonical.go. DO NOT EDIT.
//
// The product schema exactly as version 1 — the launch build — created it.
// schema/parity_test.go builds a database from this, applies every delta,
// and requires the result identical to a database built from Canonical.
// Re-run the freeze only for a pre-launch change to Canonical.
package baseline
...
// Apply creates the version-1 schema.
func Apply(app core.App) error {
	return schema.ApplyTables(app, V1)
}

EOF
    awk '/^const longTextMax/ { p = 1 } p' "$src" \
      | sed -e 's/^var Canonical = \[\]TableDef{/var V1 = []schema.TableDef{/' -e 's/\[\]IndexDef{/[]schema.IndexDef{/g' \
      | awk '
          /^\/\// { buf = buf $0 "\n"; next }
          /^var V1 = / { buf = ""; print; next }
...
case "${1:-}" in
  ...
  schema:freeze) shift; cmd_schema_freeze "$@" ;;
