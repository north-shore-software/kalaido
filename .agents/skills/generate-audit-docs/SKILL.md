...
| `reconcile.md` | Shared freshness machinery: staleness evaluation across the dependency graph, per-window evaluation for reflections, reconcile waves and every entry point that starts one, the in-process wave state published on the wire | `internal/reconcile/` (evaluator, worker); reconcile handlers |
...
| Doc | Covers | Primary source roots |
|---|---|---|
| `ingestion.md` | How content becomes fragments: entry paths, parsers, writer, birth hooks and what they signal, soft delete; the post-import pipeline that sequences mapping then discover (it is emitted here, so it is described here; `status.md` only observes it) | `internal/ingest/` (+ parsers, pipeline); fragment hook registrations |
...
| `discover.md` | The discover flows: run record and states, how each flow drives the shared tool loop, the flow kinds and their order, what each proposes vs creates, rhythm detection for reflections, kick/retry behaviour, handoff to refine | `internal/discover/`; `internal/agent/` (the shared tool loop); `internal/sourcedata/` reads |
| `explore.md` | The explore conversation: persistence, mention expansion, per-turn model resolution, stream shape, token guard; the summaries mode (selection by spec, seeding rows and map digest, read tools, read persistence and replay); bookmarks and their save-as-fragments, the projection brief, the conversation form of the prompt estimate | `internal/explore/`; `internal/chat/` (shared persistence, hydration and stream helpers); `internal/mapreader/`; `internal/agent/`; `internal/llmcontext/` summaries code; explore handlers |
| `status.md` | The derived workspace status served on one route: its axes, every state and the rule that selects it, which facts come from rows and which from the workers' in-flight flags, how a crash leftover is reported as interrupted, the surfaced policy flags, and what it does not do (no estimate, no kicking, nothing stored). Readers of the status (the onboarding splash, the dashboard) are out of scope; the workers it observes are described in `map.md`, `discover.md`, `colours.md`, `reconcile.md`, `boot-and-workers.md`; the post-import pipeline it observes is `ingestion.md` | `internal/status/`; the in-flight accessors in `internal/mapping/`, `internal/discover/`, `internal/reconcile/`, `internal/colour/`; the status handler and its wire types in `internal/api/` |
