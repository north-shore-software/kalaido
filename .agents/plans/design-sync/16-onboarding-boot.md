...
1. **Type migration without visual change** (§2 roles): these pages are ~58
...
2. **KalaidoscopeSetup double heading** (§2: one title per screen): it has two
   identical semibold headings. Demote the second (`h2`) to a `card-title` or
   `label` — Sara picks.
