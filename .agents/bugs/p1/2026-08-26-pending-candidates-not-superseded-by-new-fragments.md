...
## Update (2026-09-14)
Resolved: the speculative wave (`internal/reconcile`) is no longer parked and now runs after every event that can change a lens's inputs — an import completing, an app-written fragment, a colour drain that wrote membership, a map settle, an approve, any refinement commit, and server start. A pending candidate whose resolved context no longer matches is regenerated by the next wave (its `SnapshotIsCurrent` guard reads it as outdated), and the fresh candidate discards the old one. Covered by `internal/reconcile/worker_test.go` and `timing_test.go`.
