...
- **First-window truncation**: where a relative window's computed start would fall before **Start Time**, the window is truncated to begin at Start Time. This can only happen where **Duration** exceeds the span from Start Time to the grid point, so truncation applies **only under overlapping windows (`Duration > Period`)**, and there only to the first $\lceil \text{Duration} / \text{Period} \rceil - 1$ grid points. It never applies in tumbling or gapped configurations — in particular it does not extend a gapped configuration's first window back to Start Time (see **Duration vs. Period Relationship**).
...
- **Gapped Windows (`Duration < Period`)**: Windows sample discrete time slices (e.g., a 1-hour lookback evaluated every 24 hours), leaving un-evaluated gaps of length $\text{Period} - \text{Duration}$ — one **before the first window**, and one between each subsequent pair. An Event Date $t$ belongs to no window exactly when $(t - \text{Start Time}) \bmod \text{Period} < \text{Period} - \text{Duration}$.
  - The **leading gap is not an edge case to be truncated away**. First-window truncation never fires in this mode, so the span immediately following Start Time is itself un-evaluated: a 1-hour lookback evaluated every 24 hours from midnight produces its first window at 23:00–24:00, and fragments in the preceding 23 hours belong to no window and flag nothing. This is the intended sampling behaviour, but it is the case most likely to be misread as a defect, because Start Time reads like the beginning of coverage rather than the origin of the grid.
...
**A snapshot only clears triggers it could have seen.** Both paths resolve their inputs when the snapshot is *generated*, but the snapshot is approved later, and triggers can fire in between. A snapshot therefore clears staleness only for triggers that fired at or before the moment its `Resolved Context` was resolved (its **generation timestamp**). A trigger that fires afterwards survives approval: the snapshot is published and becomes active as normal, but the target is left stale and the engine regenerates.

This matters on both paths, and the generation-to-approval interval can be arbitrarily long:
- **Background path**: a fragment arrives while a Candidate Snapshot sits unapproved in the pending queue under Manual Approval.
- **Interactive path**: a fragment arrives while a Refinement Chat is open, after the Preview Snapshot under review was generated.

Without this rule, approving a snapshot silently discards the later trigger — the published output does not contain the new input, and no flag remains anywhere to show that it is missing. This is the only circumstance in which approving a snapshot does not leave its target fresh, and it does not weaken **Non-Destructive Staleness**: the newly approved snapshot is active and viewable throughout.

**A corollary: approving a Preview can move the active snapshot backwards.** A Preview Snapshot carries the `Resolved Context` it was generated from, and approving it supersedes whatever is active at that moment — including a *newer* Approved Snapshot the engine published while the Refinement Chat was open. The target is then correctly flagged stale by the rule above and regenerated under the newly distilled Lens, so nothing is lost; but the user briefly sees a synthesis with less input than the one it replaced. This is reachable whenever a chat is open long enough for an upstream change to land, and is most likely under **Automatic Approval**, where the engine publishes without waiting for anyone.
...
- **Context is resolved at generation time, not at poll time.** A target observed stale by the driver and regenerated a moment later resolves its `Context Spec` against the *later* state (per **Resolution & Staleness Lifecycle**). Anything that arrived in the interval is therefore included in the resulting snapshot rather than lost, and the staleness it caused is legitimately cleared by that snapshot. *(This holds because generation follows the trigger. The generation-to-approval interval runs the other way and is governed by **Resolution of Staleness**: a snapshot does not clear a trigger that fired after its context was resolved.)*
